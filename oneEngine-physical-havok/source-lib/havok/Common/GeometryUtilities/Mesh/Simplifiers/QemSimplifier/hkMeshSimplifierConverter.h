...
class hkMeshSimplifierConverter
{
	...
	HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE,hkMeshSimplifierConverter);
		...
		hkResult initMesh(const hkMeshShape* shape, hkQemSimplifier& simplifier, hkFindUniquePositionsUtil& positionUtil, const Threshold* thresholds, hkBool unitScalePosition);
			
			/// Add lots of meshes to simplify simultaneously. If not NULL, the thresholds will be passed to the vertex sharing utility.
		hkResult initMeshes(const hkMeshShape*const* srcMeshes, int numMeshes, hkQemSimplifier& simplifier, hkFindUniquePositionsUtil& positionUtil, const Threshold* thresholds, hkBool unitScalePosition);
		...
		hkMeshShape* simplifyCoplanar(hkMeshSystem* system, const hkMeshShape* meshShape, const Threshold* thresholds, hkReal maxCoplanarError = 1e-6f, hkBool allowOpen = true, hkBool unitScalePosition = true);

			/// Set the default weight for a usage
		void setDefaultWeight(hkVertexFormat::ComponentUsage usage, hkReal weight) { m_defaultWeights[usage] = weight; }
			/// Get the default weight for a usage
		hkReal getDefaultWeight(hkVertexFormat::ComponentUsage usage) const { return m_defaultWeights[usage]; }
	...
	protected:
		...
		void _verifyLastGroupAttributes(hkQemSimplifier& simplifier, const hkVertexRealConverter& converter, hkMeshMaterial* material);
