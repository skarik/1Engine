...
inline void hkpContactPointEvent::setNormal( const hkVector4& normal )
{
	...
	const hkReal estimatedDistance = m_contactPoint->getDistance() * m_contactPoint->getNormal().dot<3>( normal ).getReal();
	m_contactPoint->setSeparatingNormal( normal, estimatedDistance );

	// Set separating velocity if necessary
	if( m_separatingVelocity )
	{
		*m_separatingVelocity = hkpSimpleContactConstraintUtil::calculateSeparatingVelocity( m_bodies[0], m_bodies[1], m_bodies[0]->getCenterOfMassInWorld(), m_bodies[1]->getCenterOfMassInWorld(), m_contactPoint );
	}
}
