...
class hkInplaceVectorN: public hkVectorN
{
	public:
		...
		hkInplaceVectorN(hkInplaceVectorN const& other):
			hkVectorN(m_vecs, SIZE)
		{
			hkMemUtil::memCpy(m_vecs, other.m_vecs, sizeof(hkVector4)/4*SIZE);
		}
		
		~hkInplaceVectorN() { HK_ASSERT(0x6dadaa2d, !wasReallocated() ); }
