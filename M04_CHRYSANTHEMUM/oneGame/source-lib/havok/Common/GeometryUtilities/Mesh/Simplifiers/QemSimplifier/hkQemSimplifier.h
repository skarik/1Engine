...
class hkQemSimplifier: public hkReferencedObject
{
	public:
		...
		struct Attribute
		{
			...
			/// Start the mesh
		void startMesh(const hkAabb& aabb, const hkVector4* uniquePositions, int numPositions, hkBool unitScalePosition);
			/// Start a group. A group defines a set of triangles that share attributes.
		void startGroup(int attribSize, const AttributeFormat& fmt, hkReal positionToAttribScale);
			/// Add a triangle
		...
		HK_FORCE_INLINE void setPreserveMaterialBoundaryTolerance(hkBool enable, hkReal tol) { m_preserveMaterialBoundary = enable; m_materialBoundaryTolerance = tol; }

			/// Sets the coplanar simplification mode
		HK_FORCE_INLINE void enableCoplanarSimplify(hkBool enable, hkReal tol)	{ m_simplifyCoplanar = true; m_coplanarTolerance = tol; }
	...
	protected:
		...
		int _addGroup(int attribSize, const AttributeFormat& fmt, hkReal positionToAttribScale);
		...
		hkReal m_materialBoundaryTolerance;										///< Allows preservation of material boundaries by setting a very high cost on contractions that change it beyond this value

		hkBool m_simplifyCoplanar;												///< Only simplifies triangles that are coplanar
		hkReal m_coplanarTolerance;												///< Tolerance for co-planarity (i.e. maximum distance from a vertex to a plane for the vertex to be considered coplanar)
