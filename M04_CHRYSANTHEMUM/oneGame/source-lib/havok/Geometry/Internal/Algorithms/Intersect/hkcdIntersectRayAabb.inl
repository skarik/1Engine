...
HK_FORCE_INLINE	hkBool32	hkcdRayDirectionAabbIntersect(const hkcdRay& ray, const hkAabb& aabb, hkSimdReal* fractionInOut)
{
	...
	*fractionInOut = nearX.getComponent<0>();
	...
	for(int i=0; i<3; ++i)
	{
		const hkReal	mi = ray.m_invDirection(i) * aabb.m_min(i);
		const hkReal	mx = ray.m_invDirection(i) * aabb.m_max(i);
		if(mi > mx)
		{
...
HK_FORCE_INLINE	hkBool32	hkcdIntersectRayAabb(const hkcdRay& ray, const hkAabb& aabb, hkSimdReal* fractionOut)
{
	...
	#else
	hkReal	fmin = 0.0f, fmax = fractionOut->getReal();
	for(int i=0; i<3; ++i)
	{
													...
													const hkSimdReal& earlyOutHitFraction)  
{
	...
	#else
	hkSimdReal		f[] = {earlyOutHitFraction,earlyOutHitFraction,earlyOutHitFraction,earlyOutHitFraction};
	const int	_0 = hkcdIntersectRayAabb(ray, aabb0, &f[0]) ? hkVector4Comparison::MASK_X : 0;
	const int	_1 = hkcdIntersectRayAabb(ray, aabb1, &f[1]) ? hkVector4Comparison::MASK_Y : 0;
	const int	_2 = hkcdIntersectRayAabb(ray, aabb2, &f[2]) ? hkVector4Comparison::MASK_Z : 0;
	const int	_3 = hkcdIntersectRayAabb(ray, aabb3, &f[3]) ? hkVector4Comparison::MASK_W : 0;
	return _0 | _1 | _2 | _3;
	#endif
}
