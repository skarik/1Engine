...
function Execute ( deltaTime )
	...
	if ( m_isSearching ) then
		...
		elseif ( m_attackState == 1 ) then
			...
			print( "len: " .. vDeltaPos:length() );
			print( "circletime: " .. m_circlingTime );
			
			-- Start defending when close enough and not moving
			if ( ( m_isMoving == false ) or ( vDeltaPos:length() < 6 ) ) then	
				-- Defend
		...
		elseif ( m_attackState == 3 ) then
			...
			if ( m_AtkfeedbackWaitingState == 0 ) then
				...
				_npc_ai_FaceAt( targetAI, vSeekPosition.x,vSeekPosition.y,vSeekPosition.z );
				if ( _npc_ai_MoveTo( targetAI, m_seekPosition.x,m_seekPosition.y,m_seekPosition.z, false ) ) then
					-- request attack
	...
	print( "Current attack state: " .. m_attackState );
