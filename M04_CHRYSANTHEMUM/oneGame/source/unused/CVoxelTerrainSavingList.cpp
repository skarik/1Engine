...
void CVoxelTerrain::PushSavingList ( CBoob* pBoob, RangeVector const& position )
{
...
void CVoxelTerrain::PopSavingList ( void )
{
	if ( !vBoobSaveList.empty() )
	{
		...
		// Only save if it has data to save
		if ( topRequest.pBoob->hasBlockData && topRequest.pBoob->hasPropData )
		{
