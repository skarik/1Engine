...
void Terrain::CWorldGen_Terran::Generate_Architecture_Test ( const quickAccessData& qd )
{
	...
	*/
	...
	qd.editor->Mak_Line( Vector3d_d( 0,0,28 ), Vector3d_d( 50,50,-36 ), 5.5, EB_NONE );

	for ( uint i = 0; i < boxes.size(); ++i )
	{
		qd.editor->Add_Box( boxes[i].position, boxes[i].halfsize, EB_STONEBRICK );
	}
	...
	/*if ( qd.indexer.x == -1 && qd.indexer.y == -1 && qd.indexer.z == 0 )
	{
		...
		newprop.component_index = TerraComponent::GetComponentType( "PropCampfire" );
