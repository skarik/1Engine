	...
	: CRenderableObject ()
{
	...
	defaultMat->passinfo[0].shader = new glShader( ".res/shaders/v2d/default.glsl" );
	...
	for ( uint rt_i = 0; rt_i < rtList.size(); ++rt_i )
	{
		if ( rtList[rt_i]->GetHeight() )
		{
			...
			if ( rtList[rt_i]->GetColorSampler() )
			{
				...
				GLd.DrawRectangle( currentX, currentY, 128*(rtList[rt_i]->GetWidth()/rtList[rt_i]->GetHeight()),128 );
				currentY += 128;
			}
