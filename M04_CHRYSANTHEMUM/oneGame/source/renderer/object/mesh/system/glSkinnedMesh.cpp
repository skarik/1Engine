...
void glSkinnedMesh::FreeGPUBuffer ( void )
{
	...
	if ( m_textureBufferData != 0 )
	{
		glDeleteBuffers( 1, &m_textureBufferData );
		m_textureBufferData = 0;
	}
	if ( m_textureBufferData_Swap != 0 )
	{
		glDeleteBuffers( 1, &m_textureBufferData_Swap );
		m_textureBufferData_Swap = 0;
	}
...
void glSkinnedMesh::UpdateVBO ( void )
{
	if ( newStreamReady )
	{
		...
		// Copy data to the buffer
		glBufferData( GL_ARRAY_BUFFER,
			sizeof(CModelVertex) * (pmStreamData->vertexNum),
			pmStreamData->vertices,
			GL_STREAM_DRAW );
