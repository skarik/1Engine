		...
		do
		{
			...
			if ( currentEntry.type == MCCOSF_ENTRY_NORMAL )
			{
				if ( strcmp( currentEntry.name, "basedamage" ) == 0 ) {
					base_weapon_stats.damage = (Real)atof( currentEntry.value );
				}
				else if ( strcmp( currentEntry.name, "baserecover" ) == 0 ) {
					base_weapon_stats.recover_time = (Real)atof( currentEntry.value );
				}
				else if ( strcmp( currentEntry.name, "basereach" ) == 0 ) {
					base_weapon_stats.reach = (Real)atof( currentEntry.value );
				}
						...
						if ( currentEntry.type == MCCOSF_ENTRY_NORMAL )
						{
							if ( strcmp(currentEntry.name,"brand") == 0 ) {
								...
								if ( lastModel ) {
									...
									{
										/*if (( values[i] == "wood" && additive == ItemAdditives::Wood )
											||( values[i] == "iron" && additive == ItemAdditives::Iron )
											||( values[i] == "bone" && additive == ItemAdditives::Bone )
											)*/
