...
CFXNotice::CFXNotice ( const int ntype, const Vector3d& nposition )
{
	...
	if ( ntype == 0 ) effect_mat->loadTexture( ".res/textures/icons/notice.png" );
	if ( ntype == 1 ) effect_mat->loadTexture( ".res/textures/icons/anger.png" );
	m_effect->SetMaterial( effect_mat );
	effect_mat->removeReference();*/

	glMaterial* effect_mat = new glMaterial;
	if ( ntype == 0 ) effect_mat->setTexture( 0, new CTexture(".res/textures/icons/notice.png") );
	if ( ntype == 1 ) effect_mat->setTexture( 0, new CTexture(".res/textures/icons/anger.png") );
	...
	effect_mat->passinfo[0].shader = new glShader( ".res/shaders/v2d/default.glsl" );
