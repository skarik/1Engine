...
#include "after/terrain/generation/dungeons/DungeonBase.h"
#include "after/terrain/generation/dungeons/DungeonFactory.h"
...
// =======================================
void Terrain::CWorldGen_Terran::Generate_Caves ( const quickAccessData& qd )//( CBoob * pBoob, RangeVector const& position )
{
	while ( m_dungeonIO_flag.test_and_set() ) {
	...
	try
	{
		for ( char k = 0; k >= -3; k-- ) { // Generate downwards first
			for ( char i = -3; i <= 3; i++ ) {
				for ( char j = -3; j <= 3; j++ ) {
					TerraGen_Dungeons_Generate( RangeVector( qd.indexer.x+i,qd.indexer.y+j,qd.indexer.z+k ) );
				}
			}
		}
	...
	if ( dungeonList.size() > 1600 ) {
		...
		for ( unordered_map<RangeVector,DungeonBase*>::iterator it = dungeonList.begin(); it != dungeonList.end(); ++it )
		{
			if ( it->second ) {
				delete it->second;
				it->second = NULL;
	...
	Terrain::DungeonBase::generator = this;
	TerraGen_Dungeons_Excavate(qd);//( pBoob, position );
...
void Terrain::CWorldGen_Terran::TerraGen_Dungeons_Generate ( const RangeVector& position )
{
	RangeVector	vCheckpos;
	DungeonBase* pTargetDungeon;
	char		tempstring[256];
	...
	unordered_map<RangeVector,DungeonBase*>::iterator hashFind;
	...
	if ( hashFind != dungeonList.end() ) {
		...
		if ( oCheckfile.Exists( tempstring ) ) { // If it exists, load it and skip again
			...
			pTargetDungeon = DungeonBase::load( oCheckfile, this ); // Load dungeon
			oCheckfile.Close();
	...
	if ( dungeonSamplerList.empty() )
	{
		...
		unordered_map<short,DungeonBase*(*)(CWorldGen_Terran*)>::iterator it;
	...
	{
		std::vector<DungeonBase*>::iterator it = dungeonLocalList.begin();
				...
				for ( char k = -3; k <= 3; k++ ) {
					...
					else
					{
						...
						if ( oCheckfile.Exists( tempstring ) ) { // If it exists, then load it to the list
							...
							pTargetDungeon = DungeonBase::load( oCheckfile, this );
							oCheckfile.Close();
	...
	// Get elevation
	elevation = GetLandmassity( Vector2d_d( position.x * 64, position.y * 64 ), false );//TerraGen_1p_GetElevation( position );
	// Grab the terrain type
	char terType = TER_DEFAULT;//TerraGen_priv_GetType( Vector3d( (ftype)position.x, (ftype)position.y, (ftype)position.z ) );
	...
	//weight = maxweight * Random.Range( 0.0,1.0 );//( noise_hhf->Unnormalize(noise_hhf->Get3D( position.z * 0.7f - 1.21f, position.y * 1.2f + 2.33f, position.x * 0.13f + 2.1f )) + 0.5f );
	weight = maxweight * Math.Clamp( 
		( -100
		+m_buf_general.sampleBufferMicro( position.x*9.23461,position.y*9.23461 )
		+m_buf_general.sampleBufferMicro( position.y*8.64213,position.z*8.64213 )
		+m_buf_general.sampleBufferMicro( position.z*8.93237,position.x*8.93237 )) / 568.0f,
		0,1 );/*Random.Range( 0.0,1.0 );//( noise_hhf->Unnormalize(noise_hhf->Get3D( position.z * 0.7f - 1.21f, position.y * 1.2f + 2.33f, position.x * 0.13f + 2.1f )) + 0.5f );*/
	countweight = 0.0f;
	pTargetDungeon = NULL;
...
void Terrain::CWorldGen_Terran::TerraGen_Dungeons_Excavate ( const quickAccessData& qd )//( CBoob * pBoob, RangeVector const& position )
{
	...
	for ( unordered_map<RangeVector,DungeonBase*>::iterator it = dungeonList.begin(); it != dungeonList.end(); ++it )
	{
		...
		if ( it->second != NULL ) {
			it->second->Excavate(qd);//( pBoob, position );
		}
	}
}