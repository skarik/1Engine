...
/*
void CVoxelMesher::Cube_DC ( Real x, Real y, Real z )
{
...
void CVoxelMesher::Vertex_DC ( const CubeIndexer& index )
{
	...
	if ( m_pushdown ) {
		cube->position -= cube->normal * Terrain::BlockSize * 0.7f;
		cube->blk_smooth += 3;
	}
}

void CVoxelMesher::Triangulate_DC ( const CubeIndexer& index )
{
...
/*
void CVoxelMesher::Vertex_DC_BORDER ( const CubeIndexer& index, const uint border )
{
	//Cube *cube = &(cubes[index.index]);
	Cube* cube = &(cubes[(index.x)+(index.y)*index.sizex+(index.z)*index.sizex*index.sizey]);
	Real x = index.position.x;
