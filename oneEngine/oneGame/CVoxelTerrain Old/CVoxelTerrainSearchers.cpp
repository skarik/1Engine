...
// --Grabbin roots-
const LongIntPosition&	CVoxelTerrain::GetRootPosition ( void )
{
...
bool CVoxelTerrain::GetActiveCollision ( const Vector3d& position )
{
	...
	if ( fabs(dif.x) > iTerrainSize )
	{
		return false;
	}
	else if ( fabs(dif.y) > iTerrainSize )
	{
		return false;
	}
	else if ( fabs(dif.z) > iTerrainSize )
	{
		return false;
	}
	return true;
...
bool		CVoxelTerrain::GetBlockOnEdge ( BlockInfo const& block, EFaceDir direction )
{
	return GetBlockOnEdge( block.b16index, block.b8index, block.b1index, direction );
}
bool		CVoxelTerrain::GetBlockOnEdge ( char index16, char index8, short i, EFaceDir direction )
{
	const char width = 8;
	const char ofs = 1;
...
bool		CVoxelTerrain::SetBlock( BlockInfo const& infoes, char nData )
{
	if ( *(infoes.pBlock) != nData )
	{
		*(infoes.pBlock) = nData;
...
CWeaponItem* CVoxelTerrain::ItemizeBlock( BlockInfo const& infoes )
{
	...
	if ( *(infoes.pBlock) == EB_GRASS )
		*(infoes.pBlock) = EB_DIRT;

	// Now itemize it
	if ( *(infoes.pBlock) != EB_NONE )
		newBlok = new ItemTerraBlok ( myMat, *(infoes.pBlock) );
