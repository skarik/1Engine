...
Cubic::Cubic ( Vector3d const& vPosition, Vector3d const& vSize )
{
	position = vPosition;
	size = vSize;
}
...
// Get center
Vector3d Cubic::center ( void ) const
{
	return ( position + (size*0.5f) );
}

// Returns a frustum
Frustum Cubic::GetFrustum ( Vector3d const& casterPosition )
{
	...
	Vector3d places [8] = {
		position,
		position + Vector3d( size.x,0,0 ),
		position + Vector3d( 0,size.y,0 ),
		position + Vector3d( size.x,size.y,0 ),
		position + Vector3d( 0,0,size.z ),
		position + Vector3d( size.x,0,size.z ),
		position + Vector3d( 0,size.y,size.z ),
		position + Vector3d( size.x,size.y,size.z )
	};
...
bool Cubic::PointIsInside ( Vector3d const& point )
{
...
{
	Vector3d cubePosition = (position+size*0.5f)-(cube.position+cube.size*0.5f);
	Vector3d cubeExtents = (size + cube.size)*0.5f;
...
{
	/*Vector3d position2 = position+size;
	...
	Vector3d lineExt = Vector3d( fabs(lineDir.x), fabs(lineDir.y), fabs(lineDir.z) );
...
/*bool CCubic::LineGetCollision ( Line const& line, Vector3d & outPos )
{
	Vector3d position2 = position+size;
...
bool inline CCubic::InBox( Vector3d Hit, Vector3d B1, Vector3d B2, const char Axis )
{
...
bool rayAABBIntersect(Vector3d start, Vector3d dir, Vector3d min, Vector3d max, Vector3d& penter, Vector3d& pexit)
{
...
bool Cubic::LineGetCollision ( Line const& line, Vector3d & outPos )
{
	Vector3d bleh;
