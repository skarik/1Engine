...
XTransform	CPlayer::GetHoldTransform ( char i_handIndex )
{
	...
	rotMatrix2.setRotation( Vector3f( 0,-27,29 ) );
	result.position = ( pCamera->transform.position + (rotMatrix*(rotMatrix2*Vector3f(1.0f,0,0))) );
...
XTransform	CPlayer::GetBeltTransform ( char i_beltIndex )
{
	...
	result.position = transform.position + transform.Forward() + Vector3f( 0,0,fPlayerHeight*0.5f );
	return result;
}
*/