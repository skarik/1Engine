	...
	if ( m_currentDialogue == kElementHandleInvalid )
	{
		...
		if ( Input::Keydown( Keys.Tab ) )
		{
			...
			while ( keepCycling )
			{
				...
				{
					if ( m_elements[nextFocus]->m_visible && m_elements[nextFocus]->m_wasDrawn && m_elements[nextFocus]->canHaveFocus )
					{
						keepCycling = false;
					}
...
void Dusk::UserInterface::UpdateElements ( void )
{
	...
	{
		// Reset offset
		//Vector2f parenting_offset = Vector2f(0,0);
		std::vector<bool> offsetListGenerated(m_elements.size(), false);
		std::vector<Vector2f> offsetList(m_elements.size(), Vector2f(0,0));
