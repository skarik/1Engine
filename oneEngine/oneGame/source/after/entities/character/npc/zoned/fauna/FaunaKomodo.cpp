...
REGISTER_ZCC(FaunaKomodo);

FaunaKomodo::CONSTRUCTOR_ZCC_V2(FaunaKomodo)
{	ZCC_AddInstance();
	fadeOutValue = 3.0f;
	...
	stats.iAgility		= 1; //20+2x
	stats.iIntelligence	= 2; //5+3x
	stats.ResetStats();
	strcpy( charName, "Komodo" );
	...
	// set AI move values from stats
	//ai.ai_move.walkSpeed = 4.0f;
	//ai.ai_move.runSpeed = 11.0f;
	fWalkSpeed = 4.0f;
	fRunSpeed = 11.0f;
...
void FaunaKomodo::OnDeath ( Damage const& dmg )
{
	ai.SetIsDead( true );
...
void FaunaKomodo::OnDamaged ( Damage const& hitDamage, DamageFeedback* dmgFeedback )
{
...
void FaunaKomodo::OnInteract ( CActor* interactingActor )
{
...
bool FaunaKomodo::PerformAttack ( void )
{
...
CCharacter*	FaunaKomodo::GetCombatTarget ( void )
{
	return ai.QueryAggroTarget();
}


void FaunaKomodo::UpdateActive ( void )
{
	if ( !pRigidBody )
	{
...
void FaunaKomodo::UpdateInactive ( void )
{
	if ( pRigidBody )
	{
		delete pRigidBody;
...
void FaunaKomodo::OnAnimationEvent ( const Animation::eAnimSystemEvent eventType, const Animation::tag_t tag )
{
...
CActor* FaunaKomodo::FivePointCollision ( void )
{
...
// Move Unstuck
void FaunaKomodo::MoveUnstuck ( void )
{
...
Ray		FaunaKomodo::GetEyeRay ( void )
{
	if ( !pCharModel ) {
...
Rotator FaunaKomodo::GetAimRotator ( void )
{
...
void FaunaKomodo::MvtDefault ( void )
{
