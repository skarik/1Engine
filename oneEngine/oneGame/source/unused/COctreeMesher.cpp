...
bool COctreeMesher::m_useDualContouring = true;

COctreeMesher::COctreeMesher ( COctreeTerrain* n_terrain, COctreeRenderer* n_renderer )
	: m_terrain(n_terrain), m_renderer(n_renderer),
	m_split_border(), m_pushdown(false)
{
	m_min = Vector3d( -32,-32,-32 );
	m_max = Vector3d( 32, 32, 32 );
	m_step = COctreeTerrain::blockSize;

	//m_split_border = {false,false,false,false,false,false};
}

void COctreeMesher::CreateMesh ( CTerrainVertex* io_vertices, CModelTriangle* io_triangles, uint& o_vertex_count, uint& o_triangle_count )
{
	vertexCount = 0;
	triangleCount = 0;
	...
	bool useDualContouring = m_useDualContouring;
...
Real COctreeMesher::BlockGetSpecular ( const ushort block )
{
	switch ( block&0xFF )
	{
	case EB_MUD:
...
Real COctreeMesher::BlockGetSaturation ( const ushort block )
{
	switch ( block&0xFF )
	{
	case EB_MUD:
...
Real COctreeMesher::BlockGetGlow ( const ushort block )
{
	switch ( block&0xFF )
	{
	case EB_CURSED_DEADSTONE:
...
Real COctreeMesher::BlockGetBrightness ( const ushort block )
{
	blkflg_t flags;
...
Real COctreeMesher::BlockGetHue ( const ushort block )
{
	switch ( block&0xFF )
	{
	//case EB_GRASS:
...
Real COctreeMesher::BlockGetTexW ( const ushort block )
{
	switch ( block&0xFF )
	{
	case EB_GRASS:
...
Real COctreeMesher::BlockGetTexScale ( const ushort block )
{
	switch ( block&0xFF )
	{
	case EB_GRASS:
