...
CBoobMesh::CBoobMesh ( void )
{
	...
	if ( vertices == NULL )
	{
		Debug::Console->PrintError( "==================================\n" );
		Debug::Console->PrintError( "COULD NOT CREATE CPU VERTEX BUFFER\n" );
		Debug::Console->PrintError( "==================================\n" );
	}
	if ( quads == NULL )
	...
	{
		Debug::Console->PrintError( "==================================\n" );
		Debug::Console->PrintError( "COULD NOT CREATE CPU QUADS BUFFER.\n" );
		Debug::Console->PrintError( "==================================\n" );
		delete [] vertices;
		vertices = false;
	}
...
CBoobMesh::~CBoobMesh ( void )
{
	...
	if ( iWaterVBOverts != 0 )
		glDeleteBuffers( 1, &iWaterVBOverts );
	if ( iWaterVBOfaces != 0 )
		glDeleteBuffers( 1, &iWaterVBOfaces );
}
...
/*void CBoobMesh::UpdateRegen ( void )
{
	...
	glBindBufferARB(GL_ARRAY_BUFFER, iVBOverts);         // for vertex coordinates
	glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER, iVBOfaces); // for face vertex indexes

	glBufferDataARB( GL_ARRAY_BUFFER, sizeof(CTerrainVertex)*vertexCount, NULL, GL_STREAM_DRAW );
	glBufferDataARB( GL_ELEMENT_ARRAY_BUFFER, sizeof(CModelQuad)*faceCount, NULL, GL_STREAM_DRAW );
	...
	///glBindBufferARB(GL_ARRAY_BUFFER, iVBOverts);         // for vertex coordinates
	///glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER, iVBOfaces); // for face vertex indexes
	glBufferSubDataARB( GL_ARRAY_BUFFER, 0,vertexCount*sizeof(CTerrainVertex), vertices );
	glBufferSubDataARB( GL_ELEMENT_ARRAY_BUFFER, 0, faceCount*sizeof(CModelQuad), quads );
...
/*void CBoobMesh::GenerateCube( CBoob * pBoob, char const index16, char const index8, int const indexd, Vector3d const& pos, char const res )
{
	...
	if ( SideVisible( pBoob, index16, index8, (char*)pBoob->data[index16].data[index8].data,indexd,8, RIGHT, res, dot ) )
	{
		...
		SetFaceColors( vertices, RIGHT, GetSource( pBoob->data[index16].data[index8].data[indexd] );
