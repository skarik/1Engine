...
Maths::Cubic	CVoxelTerrain::GetAreaBoundingBox ( Vector3d const& pos, ftype area_size )
{
	Vector3d cubepos;
	...
	Maths::Cubic result ( cubepos, Vector3d( area_size,area_size,area_size ) );
	return result;
}
Maths::Cubic	CVoxelTerrain::CreateBoundingBox ( Vector3d const& pos, ftype area_size )
{
	Maths::Cubic result ( pos, Vector3d( area_size,area_size,area_size ) );
	return result;
}
...
void	CVoxelTerrain::Raycast32 ( CBoob * pBoob )
{
	if ( pBoob != NULL )
	{
		Maths::Cubic	boundingCube	= GetAreaBoundingBox( pBoob->position, 64.0f );
		Line			collisionLine;
...
void	CVoxelTerrain::Raycast16 ( char index )
{
	...
	Maths::Cubic	boundingCube	= CreateBoundingBox( targetPos, 32.0f );
	Line			collisionLine;
	collisionLine.start		= rcst_ray.pos + rcst_ray.dir*rcst_curdist;
	collisionLine.end		= rcst_ray.pos + rcst_ray.dir*rcst_maxdist;
	bool	containsLine	= boundingCube.LineCollides( collisionLine );
...
void	CVoxelTerrain::Raycast08 ( char index )
{
	...
	{
		if ( (rcst_pCurrentSubblock->data[index].data[i].block) != EB_NONE )
		{
...
void	CVoxelTerrain::Raycast01 ( Vector3d const& pos )
{
	...
	Maths::Cubic	boundingCube	= CreateBoundingBox( pos-Vector3d(0.01f,0.01f,0.01f), 2.02f );
	Vector3d		vHitPos;
	Line			collisionLine;
	collisionLine.start		= rcst_ray.pos + rcst_ray.dir*rcst_curdist;
	collisionLine.end		= rcst_ray.pos + rcst_ray.dir*rcst_maxdist;
	...
	if ( boundingCube.LineGetCollision( collisionLine, vHitPos ) )
	{
		...
		Debug::Drawer->DrawLine( rcst_pOutHitInfo->hitPos, rcst_pOutHitInfo->hitPos+Vector3d( 0.5f,0,0 ) );
		Debug::Drawer->DrawLine( rcst_pOutHitInfo->hitPos, rcst_pOutHitInfo->hitPos+Vector3d( -0.5f,0,0 ) );
		Debug::Drawer->DrawLine( rcst_pOutHitInfo->hitPos, rcst_pOutHitInfo->hitPos+Vector3d( 0,0.5f,0 ) );
		Debug::Drawer->DrawLine( rcst_pOutHitInfo->hitPos, rcst_pOutHitInfo->hitPos+Vector3d( 0,-0.5f,0 ) );
		Debug::Drawer->DrawLine( rcst_pOutHitInfo->hitPos, rcst_pOutHitInfo->hitPos+Vector3d( 0,0,0.5f ) );
		Debug::Drawer->DrawLine( rcst_pOutHitInfo->hitPos, rcst_pOutHitInfo->hitPos+Vector3d( 0,0,-0.5f ) );
	}
}
...
bool	CVoxelTerrain::BoxCollides ( Maths::Cubic const& bbox )
{
...
void	CVoxelTerrain::BoxCollide01 ( Vector3d const& pos )
{
	...
	Maths::Cubic boundingCube	= CreateBoundingBox( pos, 2.0f );
	/*Vector3d vHitPos;
	Line	collisionLine;
	collisionLine.start		= rcst_ray.pos + rcst_ray.dir*rcst_curdist;
	collisionLine.end		= rcst_ray.pos + rcst_ray.dir*rcst_maxdist;*/
