...
void Textures::ConvertFile ( const std::string& n_inputfile, const std::string& n_outputfile, const bool n_override )
{
	...
	if ( t_performConvert )
	{
		...
		// Convert the data loaded in
		Textures::ConvertData( imageData, &imageInfo, n_outputfile, t_currentTime );
...
void Textures::ConvertData ( const pixel_t* n_inputimg, const timgInfo* n_inputimg_info, const std::string& n_outputfile, const uint64_t n_timewrite )
{
	string			t_outputName = IO::FilenameStandardize( n_outputfile );
	FILE*			t_outFile = NULL;
	const pixel_t*	imageData = n_inputimg;
	...
	bpdHeader.datetime	= n_timewrite;
	...
	// Write low quality image
	{
		...
		uint32_t	pixelIndex;
		set_w = std::max<uint>( n_inputimg_info->width/16, 1 );
		set_h = std::max<uint>( n_inputimg_info->height/16, 1 );
		for ( uint x = 0; x < 16; x += 1 )
		{
			for ( uint y = 0; y < 16; y += 1 )
			{
				...
				for ( uint sx = set_w*x; sx < set_w*(x+1); sx += 1 )
				{
					...
					{
						pixelIndex = sx + sy * n_inputimg_info->width;
						if ( set_w != 1 ) {
							pixelIndex = sx;
						}
						else {
							pixelIndex = x / (16/n_inputimg_info->width);
						}
						if ( set_h != 1 ) {
							pixelIndex += sy*n_inputimg_info->width;
						}
						else {
			...
			for ( uint x = 0; x < t_width; x += 1 )
			{
				for ( uint y = 0; y < t_height; y += 1 )
				{
					...
					for ( uint sx = 0; sx < t_blocks; sx += 1 )
					{
						...
						{
							pixelTarget = (x*t_blocks+sx) + (y*t_blocks+sy)*n_inputimg_info->width;
