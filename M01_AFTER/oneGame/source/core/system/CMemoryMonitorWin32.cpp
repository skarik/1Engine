...
CMemoryMonitorWin32::CMemoryMonitorWin32 ( void )
{
	...
	iMaxStored = 1024*4;
	p_setSize = new unsigned long [iMaxStored];
...
// == Getters ==
unsigned long	CMemoryMonitorWin32::GetPeakUsage ( void )
{
	unsigned long max = p_setSize[0];
	for ( unsigned int i = 1; i < iMaxStored; i++ )
	{
		if ( p_setSize[i] > max )
			max = p_setSize[i];
	}
...
unsigned long*	CMemoryMonitorWin32::GetUsage ( void )
{
	return p_setSize;
}
