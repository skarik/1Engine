...
unsigned short CVoxelTerrain::GetFoliageType ( CFoliage* pFoliage )
{
	...
	{
		cout << "Warning: Given object '" << tname << "' has no valid Foliage type." << endl;
	}
...
CFoliage* CVoxelTerrain::CreateFoliage ( unsigned short iType, Vector3d const& vInPos, const string & iInData )
{
	CFoliage* result = NULL;
	if ( iType == 0 )
		cout << "Invalid foliage type";
	else if ( iType == 1 )
	{
