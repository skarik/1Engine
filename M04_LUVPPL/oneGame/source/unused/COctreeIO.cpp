...
Terrain::COctreeIO::COctreeIO ( const char* n_savefile, COctreeTerrain* n_terrain )
...
bool Terrain::COctreeIO::LoadSector ( Sector* o_sector, SectorGameData* o_sector_data, const RangeVector& n_sector_id, const uint32_t n_max_subdivide )
{
	...
	boost::mutex::scoped_lock rwLock ( m_writestate[f_id].m_lock );
	...
	ReadSectorGameData( o_sector_data, &mBufferIO, n_max_subdivide, n_sector_id );
...
//	if ( feof(ifile->GetFILE())!=0 ) {
//		throw std::exception( "Hit EoF on file." );
//	}
	ifile->CheckPosition();
...
bool Terrain::COctreeIO::SaveSector ( Sector* n_sector, SectorGameData* n_sector_data, const RangeVector& n_sector_id )
{
	//boost::mutex::scoped_lock rwLock ( m_wr_lock );
#if LOADER_MODULE==0
	...
	WriteSubsector( n_sector, &mBufferIO );
	WriteSectorGameData( n_sector_data, &mBufferIO, n_sector_id );
	mBufferIO.CheckPosition();
...
void Terrain::COctreeIO::WriteSubsector ( Sector* n_sector, CBufferIO* ofile )
{
	ofile->CheckPosition();
